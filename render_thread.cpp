...
void Render_thread::render_column2(int cast_degree, const int &cast_column)
{
    ...
    Ray ray2 = this->raycaster->cast_one(player_position, cast_degree+this->lookup->angle180);
    float total_length = ray.length;
//    float old_top = this->framebuffer->get_resolution().y;
    bottomOfWall = this->framebuffer->get_resolution().y;
...
//    Block old_block = this->world->get_block(ray2.block_id);
    Block old_block = this->world->get_block(this->player->get_position());
    float prev_block_top = 0;
...
//        bool is_exeeding_world_dimensions = old_block.world_position.x > this->world->get_world_dimensions().x ||
//                                            old_block.world_position.x == 0 ||
//                                            old_block.world_position.y > this->world->get_world_dimensions().y ||
//                                            old_block.world_position.y == 0;
        ...
        float newBottomOfWall = (ratio*this->player->get_height() + this->framebuffer->center.y);
//        std::cout << topOfWall << "-" << bottomOfWall << std::endl;
        float scale;
        ...
        topOfWall = newBottomOfWall - scale;
        float scale2 = this->player->get_distance_to_projection_plane()*old_block.floor_height/dist;
        float top_of_wall2 = newBottomOfWall - scale2;
        float bottom_of_wall2 = (ratio*this->player->get_height() + this->framebuffer->center.y);
        if(prev_block_top == 0)
            prev_block_top = top_of_wall2;
//        for(int i = old_bottom_of_wall; i > bottom_of_wall2; i--)
//        {
...
//        if(bottomOfWall < old_top)
//            bottomOfWall = old_top;
        int oldTop = topOfWall;
        if (topOfWall<0)
            topOfWall=0;
        if (newBottomOfWall>=this->framebuffer->get_resolution().y)
            newBottomOfWall=this->framebuffer->get_resolution().y-1;
        float uv_start = 0.0f;
...
//        }
        ...
        for(int i = topOfWall; i < newBottomOfWall; i++)
        {
            uv.y += uv_step;
            Texture *texture = nullptr;
            if(ray.hit_side == direction_up)
...
//        old_top = oldTop;
        ...
        old_bottom_of_wall = newBottomOfWall;
        if(topOfWall < old_top_of_wall)
            old_top_of_wall = topOfWall;
        prev_block_top = top_of_wall2;
//        prev_block_bottom = bottom_of_wall2;
...
//            int bottomOfWall;   // will be the staring point of floor and ceiling
            dist = total_dist;
            dist /= this->lookup->fish(cast_column);
//            int projectedWallHeight=((block.floor_height-current_height)*this->player->get_distance_to_projection_plane()/dist);
//            bottomOfWall = this->framebuffer->get_center().y+(projectedWallHeight*0.5);
//            topOfWall = this->framebuffer->get_center().y-(projectedWallHeight*0.5);
            float ratio = this->player->get_distance_to_projection_plane()/dist;
            bottomOfWall = (ratio*this->player->get_height() + this->framebuffer->center.y);
            float scale = this->player->get_distance_to_projection_plane()*block.floor_height/dist;
